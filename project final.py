...
#Login menu designing
from tkinter import *
from tkinter import messagebox
import pickle

#defining confirm function to check if user is already registered

def confirm(una,pwd):
    userinfo=open('userinfo.dat','ab+')
    userinfo.seek(0,0)
    try:
        info=pickle.load(userinfo)
    except EOFError as ee:
        pickle.dump({'username':una,'password':pwd},userinfo)
        return True
    bval=uval=0
    for i in info:
        if i==una :
...
#defining check function to display corresponding message to user
        
def check():
    uname=un.get()
    pword=pw.get()
    
    result=confirm(uname,pword)

    #message box pinting according to circumstance
    if result==True:
        messagebox.showinfo(title='Hello user',message='Welcome...Your login was successful')
        
    elif result==False:
        messagebox.showinfo(title='Hello user',message='Welcome back...Your login was successful.')
        
    elif result=='UTrue':
        messagebox.showinfo(title='Hello user',message='Enter correct password..')
...
t=Label(root,text='Login Page',font=('arial',14),bd=15)
t.pack()
form=Frame(root)
form.pack(side=TOP,fill=X)
name=Label(form,text='Username',font=('arial',14),bd=15)
pwrd=Label(form,text='Password',font=('arial',14),bd=15)
name.grid(row=1,sticky=W)
pwrd.grid(row=2,sticky=W)
n=Entry(form,textvariable=un)
p=Entry(form,textvariable=pw,show='*')
